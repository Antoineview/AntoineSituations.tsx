...
export default async function handler(
  ...
  try {
    ...
    if (!credential || !invitationId) {
      ...
      return res
        .status(400)
        .json({ message: 'Credential and invitationId are required' })
    }
    console.log('Register API: credential and invitationId received', {
      invitationId,
    })
    console.log(
      'Register API: received credential object keys:',
      Object.keys(credential),
    )
    console.log(
      'Register API: received credential.response object keys:',
    ...
    const challengeData = session.challengeData // Retrieve the stored challenge data

    console.log(
      'Register API: Challenge data retrieved from session:',
      challengeData,
    )
    ...
    if (!challengeData?.buffer) {
      ...
      return res.status(400).json({
        message: 'Authentication ceremony timed out or challenge is missing.',
      })
    }
    ...
    console.log(
      'Register API: Converted challenge to Base64URL string:',
      expectedChallengeString,
    )

    // 2. Process the passkey credential using @simplewebauthn/server immediately after getting challenge
    console.log(
      'Register API: Processing passkey credential with @simplewebauthn/server...',
    )
    ...
    const verificationOptions = {
      ...
      expectedRPID: process.env.NEXT_PUBLIC_WEB_ORIGIN?.replace(
        /^https?:\/\//,
        '',
      ) as string,
    ...
    console.log(
      'Register API: Verification options being passed to verifyRegistrationResponse:',
      {
        ...verificationOptions,
        response: {
          ...
          response: {
            ...
            clientDataJSON:
              verificationOptions.response.response.clientDataJSON.substring(
                0,
                100,
              ) + '...',
            attestationObject:
              verificationOptions.response.response.attestationObject.substring(
                0,
                100,
              ) + '...',
          },
    ...
    try {
      ...
      console.log(
        'Register API: Passkey verification successful.',
        verification,
      )
    } catch (verificationError) {
      console.error(
        'Register API: Passkey verification failed:',
        verificationError,
      )
    ...
    if (!verified || !registrationInfo) {
      console.error(
        'Register API: Verification result not as expected.',
        verification,
      )
      return res.status(400).json({ message: 'Passkey verification failed' })
    }
    ...
    console.log(
      'Register API: Full registration info:',
    ...
    if (
      ...
      console.error(
        'Register API: Missing required credential information:',
        verifiedCredential,
      )
    ...
    console.log(
      'Register API: Connecting to Sanity to check invitation status (after verification)...',
    )
    ...
    console.log(
      'Register API: Fetching invitation from Sanity (after verification)...',
      { invitationId },
    )
    const invitation = await sanityClient.getDocument(invitationId)
    console.log(
      'Register API: Sanity invitation result (after verification):',
      invitation,
    )
    ...
    console.log(
      'Register API: Checking for existing user with invitation_id (after verification)...',
      { invitationId },
    )
    const existingUsers =
      await sql`SELECT id FROM users WHERE invitation_id = ${invitationId}`
    let userId
    ...
    if (existingUsers.length > 0) {
      ...
      console.log(
        'Register API: Existing user found with ID (after verification):',
        userId,
      )
      ...
      if (invitation.used) {
        console.log(
          'Register API: User already exists and invitation already used. Registration previously completed (after verification).',
        )
      ...
      console.log(
        'Register API: User exists, but invitation not marked as used. Proceeding to store credential and mark invitation (after verification).',
      )
    } else {
      // No existing user with this invitation_id, create a new one
      console.log(
        'Register API: No existing user found, creating new user (after verification)...',
        { invitationId },
      )
      const newUser =
        await sql`INSERT INTO users (invitation_id) VALUES (${invitationId}) RETURNING id`
      userId = newUser[0].id // Assuming the result is an array with one row
      console.log(
        'Register API: New user created with ID (after verification):',
        userId,
      )
    }
    ...
    console.log(
      'Register API: Storing passkey credential in DB (after verification)...',
    )
    ...
    console.log(
      'Register API: Prepared credential data for DB insert (after verification):',
      {
    ...
    console.log(
      'Register API: Checking for existing credential for this user (after verification)...',
    )
    // Need to query using the Buffer credentialIdBuffer
    const existingCredential =
      await sql`SELECT id FROM passkey_credentials WHERE id = ${credentialIdBuffer} AND user_id = ${userId}`
    ...
    if (existingCredential.length === 0) {
      ...
      console.log(
        'Register API: Passkey credential stored in DB (after verification).',
      )
    } else {
      console.log(
        'Register API: Passkey credential already exists for this user (after verification).',
      )
    ...
    if (!invitation.used) {
      console.log(
        'Register API: Patching invitation in Sanity (after verification)...',
        { invitationId },
      )
      await sanityClient.patch(invitationId).set({ used: true }).commit()
      console.log(
        'Register API: Invitation marked as used in Sanity (after verification).',
      )
    } else {
      console.log(
        'Register API: Invitation already marked as used in Sanity (after verification).',
      )
    }

    // 5. Set the user session (Moved after verification)
    console.log('Register API: Setting user session (after verification)...', {
      userId,
    })
    // const session = await getIronSession<IronSessionData>(req, res, sessionOptions) // Session already retrieved above
    session.user = {
