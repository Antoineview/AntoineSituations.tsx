...
export default async function handler(
  ...
  try {
    ...
    console.log(
      'Verify API: Challenge data retrieved from session:',
      challengeData,
    )
    ...
    console.log(
      'Verify API: Converted challenge to Base64URL string:',
    ...
    console.log(
      'Verify API: Looking up credential in DB with ID:',
      credential.id,
    )
    ...
    if (storedCredential.length === 0) {
      console.error(
        'Verify API: Credential not found in database. Searched with Base64URL:',
        credential.id,
      )
    ...
    console.log(
      'Verify API: Verification options being passed to verifyAuthenticationResponse:',
      {
        ...verificationOptions,
        response: {
          ...
          response: {
            ...
            authenticatorData:
              verificationOptions.response.response.authenticatorData.substring(
                0,
                100,
              ) + '...',
            signature:
              verificationOptions.response.response.signature.substring(
                0,
                100,
              ) + '...',
          },
    ...
    try {
      ...
      console.error(
        'Verify API: Passkey verification failed:',
        verificationError,
      )
    ...
    if (!verified) {
      console.error(
        'Verify API: Verification result not as expected.',
        verification,
      )
      return res.status(401).json({ message: 'Passkey verification failed' })
    }
